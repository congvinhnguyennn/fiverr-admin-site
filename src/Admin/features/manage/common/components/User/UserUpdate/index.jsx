...
import {
  ...
  manageActionTypes,
} from "Admin/features/manage/action";
...
function UserUpdate() {
  ...
  const updateIndex = useSelector(
    (state) => state.manageReducer.userUpdateIndex
  );
  const usersList = useSelector((state) => state.manageReducer.usersList);
  const userUpdateData = usersList[updateIndex];

  const { birthday, name, email, phone, gender, certification, skill } =
    userUpdateData;
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      const res = await fetchUpdateUserAction({
        ...userUpdateData,
        ...customData,
      });
