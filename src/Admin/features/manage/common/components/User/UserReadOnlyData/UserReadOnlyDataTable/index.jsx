...
import { fetchDeleteUserById, manageActionTypes } from "Admin/features/manage/action";
import {
  Button,
  Dialog,
  DialogActions,
...
function createData(id, name, email, phone, birthday, gender, role, avatar) {
  return {
    id,
    name,
    email,
...
export default function UserReadOnlyDataTable() {
  ...
  const usersList = useSelector((state) => state.manageReducer.usersList);

  const dispatch = useDispatch();

  const handleClickDelete = (user) => {
    setDeleteUserId(user.id);
    setConfirmLabel(`Bạn có chắc muốn xóa người dùng ${user.name} chứ ?`);
    setIsOpenDeleteModal(true);
  };

  const handleClickUpdate = (index) => {
    dispatch(manageActionTypes.setUserUpdateIndex(index));
    dispatch(manageActionTypes.setUsersListCRUDStatus("update"));
  };
  ...
  const handleSubmitDeleteModal = async () => {
    const res = await dispatch(fetchDeleteUserById(deleteUserId));
  ...
  React.useEffect(() => {
    ...
    const mapToRow = cloneUsersList.map((user) =>
      createData(
        user.id,
        user.name,
        user.email,
  ...
  const handleChangePage = (event, newPage) => {
    dispatch(manageActionTypes.setUsersListPageIndex(newPage + 1));
  };

  const handleChangeRowsPerPage = (event) => {
    dispatch(
      manageActionTypes.setUsersListPageSize(parseInt(event.target.value, 10))
    );
    dispatch(manageActionTypes.setUsersListPageIndex(1));
  };
                ...
                (row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center" className="userRoleCell">
                        <Button className={row.role}>{row.role}</Button>
                      </TableCell>
                      <TableCell align="center">
                        {row.avatar && (
