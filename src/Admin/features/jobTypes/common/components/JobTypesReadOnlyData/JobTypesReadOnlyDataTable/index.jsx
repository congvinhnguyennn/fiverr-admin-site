...
import {
  ...
  jobTypesActionTypes,
} from "Admin/features/jobTypes/jobTypesAction";
...
export default function JobTypesReadOnlyDataTable() {
  ...
  const handleClickDelete = (jobType) => {
    ...
    setConfirmLabel(
      `Bạn có chắc muốn xóa loại công việc ${jobType.tenLoaiCongViec} chứ ?`
    );
    setIsOpenDeleteModal(true);
  };

  const handleClickUpdate = (index) => {
    dispatch(jobTypesActionTypes.setJobTypeUpdateIndex(index));
    dispatch(jobTypesActionTypes.setJobTypesListCRUDStatus("update"));
  };
  ...
  const handleSubmitDeleteModal = async () => {
    const res = await dispatch(fetchDeleteJobTypeById(deleteUserId));
  ...
  const handleChangePage = (event, newPage) => {
    dispatch(jobTypesActionTypes.setJobTypesListPageIndex(newPage + 1));
  };

  const handleChangeRowsPerPage = (event) => {
    dispatch(
      jobTypesActionTypes.setJobTypesListPageSize(
        parseInt(event.target.value, 10)
      )
    );
    dispatch(jobTypesActionTypes.setJobTypesListPageIndex(1));
  };
